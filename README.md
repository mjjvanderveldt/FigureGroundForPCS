...
Start the Timer: Click ▶ Start to begin the session.

The Task: Visually locate all scattered letters amidst the moving background noise.

Stop & Log: Once the task is complete (e.g., you have read all letters aloud), click ⏹ Stop.

Note: The time is automatically saved to your history if the duration is longer than 0.1 seconds.

The History Panel will open automatically to show your new record.

Scramble: Press Spacebar or click Randomize Positions to reshuffle the letters for a new round.
...
Browser: Open index.html in any modern web browser (Chrome, Edge, Firefox, Safari).

Internet: An internet connection is required to load the Firebase SDK and save/load history data.
